...
const FooterToolsInitiator = {
  ...
  _generateSubscribeOptions () {
    return {
      userVisibleOnly: true, // ? agar terhindar dari silent push
      applicationServerKey: this._urlB64ToUint8Array(CONFIG.PUSH_MSG_VAPID_PUBLIC_KEY) // ? VAPIDKEYS validasi oleh push service
    }
  },
  ...
  // ? pengelolaan state tombol Subscribe dan Unsubscribe
  ...
  async _isNotificationReady () {
    ...
    // ? checkGrantedpermission user
    if (!NotificationHelper._checkPermission()) {
      console.log('User did not granted the notification permission yet')
      const status = await Notification.requestPermission() // ? lakukan request permission
      if (status === 'denied') {
